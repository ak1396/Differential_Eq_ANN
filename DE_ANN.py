...
To model population growth using a differential equation, we first need to introduce some variables and relevant terms. The variable  𝑡 . will represent time. The units of time can be hours, days, weeks, months, or even years. Any given problem must specify the units used in that particular problem. The variable  𝑃  will represent population. Since the population varies over time, it is understood to be a function of time. Therefore we use the notation  𝑃(𝑡)  for the population as a function of time. If  𝑃(𝑡)  is a differentiable function, then the first derivative  𝑑𝑃𝑑𝑡  represents the instantaneous rate of change of the population as a function of time. We use the variable  𝐾  to denote the carrying capacity. The growth rate is represented by the variable  𝑟 . Using these variables, we can define the logistic differential equation. Then the logistic differential equation is
